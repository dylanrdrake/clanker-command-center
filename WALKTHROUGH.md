...
Everything in that budget is measured with `estimated_tokens`, which counts only what actually goes over the wire. `reasoning_details` is in, because it carries the signature a provider needs echoed back and is serialized into every follow-up request; the plain-text `reasoning` beside it is out, because it is display only and `skip_serializing` keeps it off the wire. Leaving the details out undercounted the tail of exactly the clankers that grow fastest — a reasoning model working through a long run of tool calls.

`carried` is the other constraint: what comes through a compaction verbatim instead of being summarized. Two kinds. A fenced block in a `user`/`assistant` message (`is_pasted_code`) is carried because nothing else holds it, and `render` skips its own ceiling for one, so the compactor is never asked to summarize a discussion around a code block with the middle cut out. A `read_file` result is carried because a coding conversation is mostly file reads — paired back to its call through `tool_call_id` to recover which path it was, since the result carries the text and the call carries the name. Two rules bound that, and both are judged against the *whole* history rather than the folded part: only the latest read of each path survives, and a read is replaced by a pointer when a write to the same path comes later in the array. Scanning only the folded span was a real bug — a file read before the seam and edited after it stayed carried verbatim, so the request held a copy the file no longer agreed with and an edit would be made against it. A later *read* drops the carried copy outright rather than pointing at it, because that read is either carried in its place or sitting in the kept tail, so the request already holds something better. Staleness answered by ordering, without touching the disk. `same_file` normalizes paths lexically for that comparison, so a mention through a symlink still misses. Everything carried goes *inside* the summary preamble rather than as messages of its own, for the same reason the summary does — the seam lands on a user message, so anything in front of it would put two in a row. `EXEMPT_BUDGET` caps the lot, newest first, because a carried message is carried in every request from here on.
...
**`tools.rs`** — the seven tools the model can call, defined as JSON schemas (`get_tool_definitions`) and executed in `execute_tool`: `write_file`, `read_file`, `list_files`, `search_files`, `replace_in_file`, `run_terminal_command`, and `web_fetch`. Results are returned as `serde_json::Value` objects and fed back to the model.
...
`search_files` was written sandboxed and then deliberately un-sandboxed, which is worth knowing before someone puts the bound back. A search walks a tree rather than naming one file, so fencing it to the working directory looks like the careful choice — but it cannot put anything out of reach while `read_file` beside it names any path it likes, and the sandbox is named, documented and surfaced as a bound on *writes*, so a refused search would have answered a read with a message about writes. What a path bound was really guarding against is a walk that never finishes, and `MAX_SEARCHED_FILES` guards that directly without blocking a search of a sibling project.

The read tools are bounded by size rather than by path, and both bounds exist because a tool result is paid for once and then carried in every request after it. `read_file` stops at `DEFAULT_READ_LIMIT` lines and `MAX_READ_BYTES` bytes, reporting `total_lines`, `last_line` and `truncated` so the model can page on with `offset` instead of assuming it saw the whole file; a file under both bounds is returned byte for byte rather than split and rejoined, because reading a file and writing it back must not silently drop a trailing newline or rewrite CRLF endings. `search_files` is capped on matches, on the length of each matching line, on the size of a file worth opening, on the directories worth entering (`SKIPPED_DIRS`, a list rather than a gitignore parser), and on files walked past.
